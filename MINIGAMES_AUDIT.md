...
|----|----------|-----------|-------------|----------------|
| F-01 | **Resolved** | [`firestore.rules`](firestore.rules) vs [`src/App.tsx`](src/App.tsx) | ~~Runtime writes `users_v3` and `leaderboards_v3` not covered by checked-in rules.~~ Repo now defines v3 + chat rules, [`firebase.json`](firebase.json), [`firestore.indexes.json`](firestore.indexes.json), [`database.rules.json`](database.rules.json), and [`FIREBASE.md`](FIREBASE.md) deploy/verify docs. **Deploy to Console still required:** `npm run firebase:deploy`. |
